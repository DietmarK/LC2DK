    ...
    : QWidget(parent, fl)
{
   ...
   // std::cout << "QG_ArcTangentialOptions::QG_ArcTangentialOptions\n";
...
void QG_ArcTangentialOptions::destroy() {
   ...
   RS_SETTINGS->writeEntry("/ArcTangentialType", arctype);             /* +++++++++++++++++ */
...
{
   // std::cout << "QG_ArcTangentialOptions::setAction update = " << update << "\n";
 ...
 */
	  {
	     ...
	     arctype = RS_SETTINGS->readNumEntry("/ArcTangentialType", 3);        /* ++++++++++++++++ */
	     
            RS_SETTINGS->endGroup();
            
	     action->setRadius(sr.toDouble());
            action->setAngle(sa.toDouble()*M_PI/180.);
            action->setArcType(arctype);
        }
	
	// std::cout << "QG_ArcTangentialOptions::setAction arctype = " << arctype << "\n";
	
        leRadius->setText(sr);
        leAngle->setText(sa);
        updateByRadius(0 == arctype - 1);
        updateByAngle(0 == arctype - 2 );
	
	if(arctype == 3)
	  rbEndpoint->setChecked(true);	  
...
void QG_ArcTangentialOptions::updateByAngle(const bool br) {
...
void QG_ArcTangentialOptions::on_leAngle_textEdited(const QString &arg1)
{
   if(rbAngle->isChecked())
	...
	if(remainder(d, 2. * M_PI) < RS_TOLERANCE_ANGLE)
...
void QG_ArcTangentialOptions::on_rbEndpoint_clicked(bool /*checked*/)
{
   arctype = 3;
   action->setArcType(3);
   updateByRadius(false);
