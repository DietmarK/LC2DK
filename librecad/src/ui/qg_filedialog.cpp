...
//# KDE returns the first filter that match the pattern "*.dxf" instead the selected
    ...
    QDialogButtonBox::ButtonLayout layoutPolicy = QDialogButtonBox::ButtonLayout(this->style()->styleHint(QStyle::SH_DialogButtonLayout, 0, this));
      ...
      int noNativeDialog = RS_SETTINGS->readEntry("/NoNativeDialog", "0").toInt();
      RS_SETTINGS->endGroup();
   // if (noNativeDialog>0)
     // fileDlg->setOption (QFileDialog::DontUseNativeDialog);
      /* ++++++++++++ */

   if ((layoutPolicy == QDialogButtonBox::KdeLayout) and (noNativeDialog == true))   /* true or false ?? */
     {
	setOption ( QFileDialog::DontUseNativeDialog, true );
	// std::cout << " DontUseNativeDialog, true \n";
     }
   else
     {
        setOption ( QFileDialog::DontUseNativeDialog, false );
	// std::cout << " DontUseNativeDialog, false \n";	
     } 
