...
void Plugin_Entity::getData(QHash<int, QVariant> *data){
   ...
   /* +++++++++++++++++++++++ */
   data->insert(DPI::VISIBLE, entity->isVisible());
   /* ++++++++++++++++++++++ */
   ...
   switch (et) {
    ...
    case RS2::EntityArc: {
        ...
        data->insert(DPI::REVERSED, d.reversed );   /* ++++++++++++++ is now in LC2 */
        break;}
...
RS_Pen Doc_plugin_interface::getCurrentLayerPen()
{  
          return doc->getActiveLayer()->getPen();
}


void Doc_plugin_interface::setCurrentLayerPen(RS_Pen pen)
{   
   RS_Layer* layer = doc->getActiveLayer();
   
   // std::cerr << "doc_plugin_interface: name " <<  layer->getName().toStdString() << "\n";
...
void Doc_plugin_interface::activateLayer(const QString& name)
{   
      doc->activateLayer(name);
}


// in  .h  und  /src/plugins/document_interface.h   eintragen !
...
/* ++++++++++++++++++++++++++++ */
bool Doc_plugin_interface::getAllEntities(QList<Plug_Entity *> *en)
{     
   bool status = false;
