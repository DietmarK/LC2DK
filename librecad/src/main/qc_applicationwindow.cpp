...
void QC_ApplicationWindow::initActions(void)
{
   ...
   action = actionFactory.createAction(RS2::ActionOptionsGeneralAdd, this);
    menu->addAction(action);
   //action->addTo(menu);
...
//    connect(this, SIGNAL(windowsChanged(bool)), previousZoom, SLOT(setEnabled(bool)));
   ...
   action = actionFactory.createAction(RS2::ActionSelectCircles, actionHandler);
          menu->addAction(action);
          connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
         
   action = actionFactory.createAction(RS2::ActionSelectCirclesInWindow, actionHandler);
          menu->addAction(action);
          connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
       
   action = actionFactory.createAction(RS2::ActionDeselectOutsideWindow, actionHandler);
          menu->addAction(action);
          connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
   ...
   action = actionFactory.createAction(RS2::ActionSelectOutsideCircle, actionHandler);
             menu->addAction(action);
             connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
      
   action = actionFactory.createAction(RS2::ActionDeselectInsideCircle, actionHandler);
          menu->addAction(action);
          connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
...
//    subMenu->setObjectName("Point");
   ...
   action = actionFactory.createAction(RS2::ActionDrawArcTangential, actionHandler);
    ...
    connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
      ...
      /* ++++++++++++++++++++++++++++ */
   action = actionFactory.createAction(RS2::ActionDrawArcTangentialEndpoint, actionHandler);
...
//    subMenu->setObjectName("Spline");
   ...
   /* +++++++++++++++++++++++ */
   action = actionFactory.createAction(RS2::ActionDimOrdinate, actionHandler);
   menu->addAction(action);
   connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
    ...
    connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
      ...
      /* +++++++++++++++++++++++++++++++ */
   action = actionFactory.createAction(RS2::ActionModifyMoveZero, actionHandler);
       menu->addAction(action);
       connect(this, SIGNAL(windowsChanged(bool)), action, SLOT(setEnabled(bool)));
      /* ++++++++++++++++++++++++++++ */
   ...
   // Info actions:
    ...
    action = actionFactory.createAction(RS2::ActionLayersDefreezeAll,
                                        actionHandler);
      /* +++++++++++++++++++++++++++++++++ */
      QList<QKeySequence> LShortcuts;
   LShortcuts<<QKeySequence(Qt::CTRL + Qt::Key_L)<<QKeySequence(Qt::CTRL + Qt::Key_Y);
      action->setShortcuts(LShortcuts);
      /* ++++++++++++++++++++++++++++++ */
...
void QC_ApplicationWindow::initMenuBar() {
   ...
   //openedFiles.clear();
      ...
      int RecentFiles = RS_SETTINGS->readEntry("/NumberRecentFiles", "9").toInt();
      RS_SETTINGS->endGroup();
      
      //std::cerr << "NumberRecentFiles " << RecentFiles << "\n";
      recentFiles = new QG_RecentFiles(RecentFiles);
      openedFiles.clear();
     /* +++++++++++++++++++++++++++++++ */
...
//        QString text = tr("&%1 %2").arg(i + 1).arg(recentFiles->get(i));
       ...
       /* +++++++++++++++++++++++++++++++ */
      RS_SETTINGS->beginGroup("/Privat");
      int OldestOnTop = RS_SETTINGS->readEntry("/OldestOnTop", "0").toInt();
      RS_SETTINGS->endGroup();

       if(OldestOnTop == 1)
	 {
	    QString text = tr("&%1 %2").arg(i + 1).arg(recentFiles->get(i));
	   // qDebug() << "if" << OldestOnTop << "  " << text;
...
void QC_ApplicationWindow::slotFileClosing() {
   ...
   QC_MDIWindow* w = getMDIWindow();   /* ooooooooooooooo */
   if(w!=NULL)
     openedFiles.removeAll(w->getDocument()->getFilename());
}
...
/**
 *  * Shows the dialog for additional preferences.
 *  */
void QC_ApplicationWindow::slotOptionsGeneralAdd()
{   
   RS_DIALOGFACTORY->requestOptionsGeneralDialogAdd();
    ...
    if (d!=NULL) {
        ...
        for (RS_Entity* e=d->firstEntity();
            ...
            case RS2::EntityDimOrdinate: {
                    RS_DimOrdinate* d = (RS_DimOrdinate*)e;
                    dumpFile
                    << "<table><tr><td>"
                    << "<b>Dimension / Ordinate:</b>"
                    << "</td></tr>";
                    dumpFile
                    << "<tr>"
                    << "<td>"
