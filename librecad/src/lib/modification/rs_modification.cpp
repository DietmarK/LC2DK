                           ...
                           bool both) 
{
  ...
  if (limitEntity->isAtomic()) 
  ...
  else if (limitEntity->isContainer())
    {
      ...
      for (RS_Entity* e=ec->firstEntity(RS2::ResolveAll); e!=NULL; e=ec->nextEntity(RS2::ResolveAll))
	{
  ...
  if (trimEntity->rtti()==RS2::EntityCircle) 
    {
      ...
      /* +++++++++++++++++++++++++++ */
      /* create new arc on the same layer !  */
      RS_Layer* lay = c->getLayer();
  ...
  //std::cout<<"sol "<<sol<<"\n";
  //std::cout<<"trimmed1->getTrimPoint "<<ending<<"\n";
...
/**
 * Moves all entities on all layers, keep selection, zoomPan:
 * all entities stay on the old place, only the absolute zero moves
 * to the new position
 */
bool RS_Modification::moveZero(RS_MoveData& data) 
{
   if (container==NULL) 
     {
        RS_DEBUG->print("RS_Modification::move: no valid container",
...
//	  }
   ...
   dx = (int)graphicView->toGuiDX(data.offset.x);
   
   dy = (int)graphicView->toGuiDX(data.offset.y);
   
   graphicView->zoomPan(-dx, dy);
