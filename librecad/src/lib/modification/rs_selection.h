...
public:
    ...
    void selectWindow(const RS_Vector& v1, const RS_Vector& v2,
		       ...
		       bool outside=false);   /* +++++++++ parameter 5 ++++++++ */
   
   void deselectWindow(const RS_Vector& v1, const RS_Vector& v2,
		       bool select, bool cross=false,
		       bool outside=false) /* +++++++++ parameter 5 ++++++++ */
     {
	Q_UNUSED(select);
	selectWindow(v1, v2, false, cross, outside); /* +++++++++ parameter 5 ++++++++ */
     }
   
   
   /* ++++++++++++++++++++++++ */
   void selectCircle(const RS_Vector& cc, const double radius,
		     bool select=true, bool cross=false,
		     bool outside=false);
   
   void deselectCircle(const RS_Vector& cc, const double radius,
		       bool select, bool cross=false,
		       bool outside=false)
     {	
	Q_UNUSED(select);
	selectCircle(cc, radius, false, cross, outside);
     }
   ...
   /* +++++++++++++++++++++++++ */
    ...
    void deselectLayer(QString& layerName) {
      ...
      /* +++++++++++++++++++++ */
      void selectCircles(RS_Entity* e);
      
      void selectCircles(RS_Entity* e, RS_Entity* tk);
      
      void selectCircles(RS_Entity* e, RS_Vector v1, RS_Vector v2);
      /* +++++++++++++++++++++ */
