...
void RS_FilterDXFRW::addLWPolyline(const DRW_LWPolyline& data) {
   ...
   for (unsigned int i=0; i<data.vertlist.size(); i++) {
      ...
      RS_Vector *v = new RS_Vector(vert->x, vert->y);   /* oooooooooooooo */
...
void RS_FilterDXFRW::addPolyline(const DRW_Polyline& data) {
   ...
   for (unsigned int i=0; i<data.vertlist.size(); i++) {
      ...
      //RS_Vector v(vert->basePoint.x, vert->basePoint.y);
        //polyline->addVertex(v, vert->bulge);
      RS_Vector *v = new RS_Vector(vert->basePoint.x, vert->basePoint.y);   /* oooooooooooooo */
      verList.append(qMakePair(v, vert->bulge));
...
RS_DimensionData RS_FilterDXFRW::convDimensionData(const  DRW_Dimension* data) {
   ...
   /* ++++++++++++++++++++++++++++++++++ */
   //std::cout << "rs_filterdxfrw.cpp Text as unicode:" << qPrintable(t) << "\n";
   ...
   //std::cout << "rs_filterdxfrw.cpp Text nach replace:" << qPrintable(t) << "\n";
   /* +++++++++++++++++++++++++++++++++++++ */
...
void RS_FilterDXFRW::addDimOrdinate(const DRW_DimOrdinate* data) 
{
  ...
  /* ++++++++++++++++++++++++++++++++++++++++ */
  // std::cout << "RS_FilterDXFRW::addDimOrdinate(const DL_DimensionData&, const DL_DimOrdinateData&) not yet implemented \n"; 
  ...
  if (false)
    {
      ...
      if (data->type & 64)	   /* 64 = Ordinate type (bit 7)  If set, ordinate is X-type; if not set, ordinate is Y-type. */
	a = "\n ordinate is X-type";
      else
	a = "\n ordinate is Y-type";
      ...
      std::cout << " text winkel " << data->getDir() << "\n";
      
      std::cout << " getDefPoint x y  " << data->getDefPoint().x << "  " << data->getDefPoint().y << "\n";
      
      
      std::cout << " getTextPoint() x   " << data->getTextPoint().x << "  " <<  data->getTextPoint().y  << "\n";
      
      std::cout << " int getAlign()   " << data->getAlign()    << "\n";
      
      std::cout << " int getTextLineStyle()   " << data->getTextLineStyle()    << "\n";
      
      std::cout << " string getText()   " << data->getText()    << "\n";
      
      std::cout << " double getTextLineFactor()   " << data->getTextLineFactor()    << "\n";
      
      std::cout << " double getDir()   " << data->getDir()    << "\n";
      
      std::cout << " getExtrusion() x y  " << data->getExtrusion().x << "  " <<  data->getExtrusion().y  << "\n";
      
      std::cout << " string getName()   " << data->getName()    << "\n";
      
      
      std::cout << " ordinate: getOriginPoint() x y  " << data->getOriginPoint().x << "  " << data->getOriginPoint().y   << "\n";
      
      std::cout << " ordinate: getFirstLine() x y  " << data->getFirstLine().x << "  " << data->getFirstLine().y   << "\n";
      
      std::cout << " ordinate: getSecondLine() x y  " << data->getSecondLine().x << "  " << data->getSecondLine().y   << "\n";
      
      
      
      std::cout << " Dimension style, code 3 :: " <<  data->getStyle() << "\n\n";
    }
  ...
  RS_Vector ext1(data->getOriginPoint().x, data->getOriginPoint().y);
  RS_Vector ext2(data->getFirstLine().x, data->getFirstLine().y);
  RS_Vector ext3(data->getSecondLine().x, data->getSecondLine().y);
  
  bool is_x_type = false;
  
  if (data->type & 64)  is_x_type = true;
  
  RS_DimOrdinateData d(ext1, ext2, ext3, is_x_type);
  ...
  // std::cout << " RS_DimOrdinate  " << *entity << "\n";
...
void RS_FilterDXFRW::writeSpline(RS_Spline *s) {
    ...
    if (version==1009) {
        ...
        if (s->isClosed()) {
            pol.flags = 1;
        } else {		       /* commit 1b926f1830b608b1ee6742501c430b7272eb298a */
...
void RS_FilterDXFRW::writeDimension(RS_Dimension* d) {
    ...
    case RS2::EntityDimOrdinate: {
      ...
      //std::cout << " case RS2::EntityDimOrdinate:  \n";
      
      
      RS_DimOrdinate* d_o = (RS_DimOrdinate*)d;
        DRW_DimOrdinate * dd = new DRW_DimOrdinate();
        dim = dd ;
        dim->type = 6 +32;
        if(d_o->getXType())  dim->type += 64;
        dd->setOriginPoint(DRW_Coord (d_o->getOriginPoint().x, d_o->getOriginPoint().y, 0.0));
        dd->setFirstLine(DRW_Coord (d_o->getFirstLine().x, d_o->getFirstLine().y, 0.0));
        dd->setSecondLine(DRW_Coord (d_o->getSecondLine().x, d_o->getSecondLine().y, 0.0));
      
      //std::cout << " case RS2::EntityDimOrdinate: getXType() " << d_o->getXType() << " \n";
  ...
  // std::cout << "  vor dxf->writeDimension(dim);  \n";
...
void RS_FilterDXFRW::writeImage(RS_Image * i) {
    ...
    DRW_ImageDef *imgDef = dxf->writeImage(&image, i->getFile().toUtf8().data());   /* ooooooooooo */
    if (imgDef != NULL) {
        imgDef->loaded = 1;
...
//DRW_Entity RS_FilterDXFRW::getEntityAttributes(RS_Entity* /*entity*/) {
  ...
  //  std::cout << " RS_FilterDXFRW::getEntityAttributes pen = " << pen  << "\n";
  ...
  // std::cout << " RS_FilterDXFRW::getEntityAttributes layerName = " << qPrintable(layerName) << "\n";
  
    ent->layer = toDxfString(layerName).toUtf8().data();
  
  //  std::cout << " RS_FilterDXFRW::getEntityAttributes color = " << color << "\n";
