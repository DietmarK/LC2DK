					     ...
					     const RS_Vector& p2, bool arrow1, bool arrow2, bool forceAutoText) 
{
  ...
  textWidth = text->getUsedTextWidth();         /* ++++++++++++++++++ */
  ...
  if (getAlignText()) 
      ...
      RS_Line l[] = 
	{
	  ...
	  RS_Line()		       /* +++++++++++++++++++++++++ quick and dirty hack, see some lines later */
	    			       /* we should not remove the 5. element !!  sometimes it crashes ! */
        };
      RS_VectorSolutions sol1, sol2;
      int inters= 0;
      
      /* +++++++++++++++++++++++++ */
      /* buggy code ! second "do ... while" starts with "l[4]" when the first loop runs 4 times  */
      
      do 
	{
	  //std::cout << " rs_dimension.cpp inters = " << inters << "\n";
	  //std::cout << "RS_Line x " << l[inters].getData().endpoint.x << "  y " << l[inters].getData().endpoint.y << "\n";
      ...
      if (!sol1.hasValid()) 
	{
      ...
      /* commit f476c4f1d0bc763f25be62852fc61223835050df  #f476c4f1d0 */
...
double RS_Dimension::getTextWidth()    /* +++++++++++++++++ */
{
  double dimtxt = getTextHeight();
  RS_MTextData textData;
  RS_Vector textPos(0.0, 0.0);
