...
/* ++++++++++++++++++++++++++++ */
/**
 *  * @return True if the entity is completely outside given range.
 *  */
bool RS_Entity::isOutsideWindow(RS_Vector v1, RS_Vector v2)
{
   ...
   bottom = std::min(v1.y, v2.y);
       ...
       return (getMin().x > right || getMax().x < left
	                   || getMin().y > top || getMax().y < bottom);
}
