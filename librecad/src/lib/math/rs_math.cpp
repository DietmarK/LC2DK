...
double RS_Math::getAngleDifference(double a1, double a2, bool reversed) {
  ...
  if(1)	       /* buggy commit from 25.2.2013 ???  ( bug in RS_Arc::getTrimPoint(..) )  */
    {
      if(reversed)
...
bool RS_Math::simultaneousQuadraticVerify(const std::vector<std::vector<double> >& m, const RS_Vector& v)
{
    ...
    /**
      ...
      */
    //QVector<double> terms0={ a*x2, b*x*y, c*y2, d*x, e*y, f, g*x2, h*x*y, i*y2, j*x, k*y, l};   /* won't compile with older g++ */
