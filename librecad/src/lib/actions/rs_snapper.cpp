...
void RS_Snapper::init() {
  ...
  SnapDK = RS_SETTINGS->readEntry("/SnapDK", "0").toInt();
   AutoSnapOnEndpoint = RS_SETTINGS->readEntry("/AutoSnapOnEndpoint", "1").toInt();
   AutoSnapOnIntersection = RS_SETTINGS->readEntry("/AutoSnapOnIntersection", "1").toInt();
...
RS_Vector RS_Snapper::snapPoint(QMouseEvent* e)
{
  ...
  if(!SnapDK)
  ...
  else 
    {
      ...
      double ds2Min=RS_MAXDOUBLE;
      
      if(SnapDK && snapRange > 0)
	ds2Min = snapRange * 4 / graphicView->getFactor().x;    /* +++++++++++++++++++++ */
	...
	{
	  //RS_SETTINGS->beginGroup("/Privat");
	  //int AutoSnapOnMiddle = RS_SETTINGS->readEntry("/AutoSnapOnMiddle", "1").toInt();
	  //RS_SETTINGS->endGroup();
	  
	  if(AutoSnapOnMiddle and RS_DIALOGFACTORY != NULL) 
	    {		     
	      RS_DIALOGFACTORY->requestSnapMiddleOptions(middlePoints, snapMode.snapAuto);
	    }
	  
	  t = snapAuto(mouseCoord);   
	  ds2Min = 0.0;
	  snapSpot = t;
	}
      else if (snapMode.snapEndpoint)
	{
	  vec = container->getNearestEndpoint(mouseCoord, &ds2);
      ...
      else if (snapMode.snapCenter) 
	{
	  vec = container->getNearestCenter(mouseCoord, &ds2);
      ...
      else if (snapMode.snapMiddle)
	{
	  ...
	  vec = container->getNearestMiddle(mouseCoord, &ds2, middlePoints);
      ...
      else if (snapMode.snapDistance) 
	{
	  ...
	  vec = container->getNearestDist(distance, mouseCoord, &ds2);
      ...
      else if (snapMode.snapIntersection) 
	{
	  vec = container->getNearestIntersection(mouseCoord, &ds2);
      ...
      else if (snapMode.snapOnEntity) // && snapSpot.distanceTo(mouseCoord) > snapMode.distance  ??
	{
	  vec = container->getNearestPointOnEntity(mouseCoord, true, &ds2, &keyEntity);
...
double RS_Snapper::getSnapRange() const
{
  ...
  double sr;
  RS_SETTINGS->beginGroup("/Snap");
  sr = RS_SETTINGS->readNumEntry("/Range", 20);
  RS_SETTINGS->endGroup();
...
RS_Vector RS_Snapper::snapAuto(const RS_Vector& coord)
{
  ...
  if(AutoSnapOnEndpoint)
    {	
      t = container->getNearestEndpoint(coord, &dist);
      
      if (coord.distanceTo(t) < coord.distanceTo(vec))
	{	     
	  vec = t;
	  mindist = dist;
	  autoSnapCursorType = 1;
	}	
    }
  
  
  if(AutoSnapOnIntersection)
    {	
      t = container->getNearestIntersection(coord, &dist);
      
      if (coord.distanceTo(t) < (coord.distanceTo(vec) - 0.0000002))
	{
	  vec = t;
	  mindist = dist;
	  autoSnapCursorType = 2;
	}	
    }
  ...
  if(AutoSnapOnMiddle)
    {	
      t = container->getNearestMiddle(coord, &dist, middlePoints);
      
      if (coord.distanceTo(t) < coord.distanceTo(vec) - 0.0000003)
	{	     
	  vec = t;
	  mindist = dist;
	  autoSnapCursorType = 3;
	}
  ...
  if(AutoSnapOnCenter)
    {
      ...
      RS_Entity* en = container->getNearestEntity(coord, NULL, RS2::ResolveNone);
      
      if(en != NULL && en->isVisible())
	{		           
	  t = en->getNearestCenter(coord, &dist);
	  
	  if (coord.distanceTo(t) < coord.distanceTo(vec) - 0.0000004)
	    {
	      vec = t;
	      mindist = dist;
	      autoSnapCursorType = 4;
	    }
	}
    }
...
RS_Entity* RS_Snapper::catchEntity(const RS_Vector& pos,
  ...
  // 
  dist = graphicView->toGraphDX(snapRange)*0.9;   /* ++++++++++++++++++++++++++++ */
...
{  
  ...
  /* autoSnap: show different cursor appearance  */
  
  /*
  RS_SETTINGS->beginGroup("/Privat");
  int SnapCursorDiameter = RS_SETTINGS->readEntry("/SnapCursorDiameter", "6").toInt();
  QColor SnapCursorColor(RS_SETTINGS->readEntry("/SnapCursorColor", "#ffaa00"));
  
  int AutoSnapCursorSize = RS_SETTINGS->readEntry("/AutoSnapCursorSize", "6").toInt();
  QColor AutoSnapCursorColor(RS_SETTINGS->readEntry("/AutoSnapCursorColor", "#ffaa00"));
  
  RS_SETTINGS->endGroup();
  */
  
  double mc_sqare = AutoSnapCursorSize;
  double mc_x = mc_sqare * 1.7;
      ...
      if (snapCoord.valid) 
	{
	  ...
	  if ( ! snapMode.snapAuto )
	    {		
	      if(OK)
		{
		  pen = new RS_Pen(RS_Color(SnapCursorColor), RS2::Width00, RS2::SolidLine);
	    ...
	    {
	      RS_Pen x_pen(RS_Color(AutoSnapCursorColor), RS2::Width00, RS2::SolidLine);
	      
	      RS_OverlayLine *line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(-mc_x,-mc_x),
									graphicView->toGui(snapSpot)+RS_Vector(-1,-1)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_x,mc_x),
							graphicView->toGui(snapSpot)+RS_Vector(1,1)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_x,-mc_x),
	  ...
	  else if (autoSnapCursorType == 1)
	    {
	      RS_Pen x_pen(RS_Color(AutoSnapCursorColor), RS2::Width00, RS2::SolidLine);
	      
	      
	      
	      RS_OverlayLine *line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(-mc_sqare,-mc_sqare),
									graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,-mc_sqare)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,-mc_sqare),
							graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,mc_sqare)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,mc_sqare),
	  ...
	  else if (autoSnapCursorType == 3)
	    {
	      RS_Pen x_pen(RS_Color(AutoSnapCursorColor), RS2::Width00, RS2::SolidLine);
	      
	      RS_OverlayLine *line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot),
									graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,-mc_x)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,-mc_x),
							graphicView->toGui(snapSpot)+RS_Vector(mc_sqare,mc_x)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot),
							graphicView->toGui(snapSpot)+RS_Vector(-mc_sqare,-mc_x)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(-mc_sqare,-mc_x),
	  ...
	  else if (autoSnapCursorType == 4)
	    {
	      RS_Pen x_pen(RS_Color(AutoSnapCursorColor), RS2::Width00, RS2::SolidLine);
	      
	      RS_OverlayLine *line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(mc_x,0),
									graphicView->toGui(snapSpot)+RS_Vector(0,mc_x)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(0,mc_x),
							graphicView->toGui(snapSpot)+RS_Vector(-mc_x,0)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(-mc_x,0),
							graphicView->toGui(snapSpot)+RS_Vector(0,-mc_x)));
	      line->setPen(x_pen);
	      container->addEntity(line);
	      line=new RS_OverlayLine(NULL, RS_LineData(graphicView->toGui(snapSpot)+RS_Vector(0,-mc_x),
