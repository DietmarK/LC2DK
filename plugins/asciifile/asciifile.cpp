...
#include <QVBoxLayout>
#include <QHBoxLayout>
#include <QCheckBox>		       /* ooooooooooooooooo */
    ...
    QGroupBox(title, parent)
{
   ...
   rb->setTristate (false );	       /* ooooooooooo */
...
//readSettings();
    ...
    txtformats << tr("Space Separator") << tr("Tab Separator") << tr("Comma Separator") 
     << tr("Space in Columns") << tr("*.odb for Psion 2");
...
//Warning, can change adding or reordering "formatedit"
    if (formatedit->currentIndex() == 4)   /* ooooooooooo */
   ...
   currDoc->setLayer(currlay);	       /* ooooooooooo */
   
   /* draw lines in current layer */
   if ( connectPoints->isChecked() )
     drawLine();
   
   // currDoc->setLayer(currlay);      /* oooooooooooo */
    currDoc = NULL;
...
void dibPunto::readSettings()
 {
    ...
    connectPoints->setChecked( settings.value("connectpoints", false).toBool() );   /* oooooooooo */
...
void dibPunto::writeSettings()
 {
    ...
    settings.setValue("connectpoints", connectPoints->isChecked());   /* ooooooooo */
